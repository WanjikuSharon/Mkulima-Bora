...
//CAUSING ERROR OF NOT SENDING THE OTP
    //while(list(,$line) = @each($lines)) {
    //  $lines_out = null;
      //if($line == "" && $in_headers) {
        //$in_headers = false;
      //}
      //NEW CODE, LETS SEE IF IT WORKS
      foreach ($lines as $line) {
        $lines_out = null;  // Initialize or reset lines_out for each line
        if ($line == "" && $in_headers) {
            $in_headers = false;  // Set $in_headers to false if the line is empty and currently in headers
        }
    }
      ...
      // send the lines to the server - COMMENTED OUT BECAUSE OF EACH ERROR
...
// Send the lines to the server NEW CODE SNIPPET
foreach ($lines_out as $line_out) {
  if (strlen($line_out) > 0) {
      if (substr($line_out, 0, 1) == ".") {
          $line_out = "." . $line_out;  // Prepend a dot if the line starts with one
      }
  }
  fputs($this->smtp_conn, $line_out . $this->CRLF);  // Send the line to the SMTP server
}
